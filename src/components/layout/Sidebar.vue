...
const temRole = (roles = []) => {
  ...
  return roles.some(role => userRoles.includes(`ROLE_${role}`))
}
...
const menuItemsBase = [
  {
    ...
    iconPath: 'M4 6h16M4 10h16M7 14h10m-8 4h6'
  },
  {
    label: 'Dashboard',
...
const menuItems = computed(() => menuItemsBase.filter(item => temRole(item.roles)))
</script>
